# Data Type Analyzer with AI-Enhanced Type Inference

This project is a web application for analyzing and converting data types in datasets, with a focus on intelligent type inference and complex data handling.


## Getting Started

### Prerequisites
- Docker & Docker Compose (for Docker setup)
- Python 3.9+ (for manual setup)
- Node.js 18+ (for manual setup)


### Installation and Running

```bash
1. Using Docker
# Clone the repository
git clone https://github.com/L1TangDingZhen/Data_Analysis

# Move to folder
cd Data_Analysis

# Start the application using Docker Compose
docker-compose up --build


2. Mannual Running

# Clone the repository
git clone https://github.com/L1TangDingZhen/Data_Analysis

# Move to Project
cd Data_Analysis

# Move to back end and run
cd back/analysis
conda create -n dataenv python=3.9
conda activate dataenv
pip install -r requirements.txt
python -m spacy download en_core_web_md
python manage.py runserver

# Move to front-end and run
cd front
npm install
npm start

```

# How to Use

1. Visit the Website localhost:3000 to access service
2. Upload your CSV or Excel file using the interface
3. Review automatically inferred data types
4. Modify types if needed using the dropdown menu
5. Export your processed data

## Use Cases

### Common Data Type Issues Solved
- Mixed date formats (e.g., "1990-01-01", "01/01/1990", "Jan 1, 1990")
- Inconsistent boolean values (e.g., "YES/NO", "1/0", "True/False")
- Complex height/weight formats (e.g., "5'11"", "180cm", "5 feet 11 inches")
- Mixed numeric formats (e.g., currency with symbols, percentages)
- Categorical data with multiple representations (e.g., gender: "M/F", "Male/Female", "Boy/Girl")

## Core Algorithms

### Intelligent Type Inference
- Initial complexity detection based on data patterns
- Rule-based preliminary type checking
- SpaCy model-based analysis for complex cases
- Confidence scoring system for type predictions
- Type conversion with fallback mechanisms


## API Documentation

### Endpoints
- `POST /analyze/`: Upload and analyze file
  - Returns: Column types, sample values, and inference info
- `POST /update-type/`: Update column type
  - Parameters: column, new_type, file_id
- `GET /export/{file_id}/`: Export processed data

## Configuration
### Environment Variables
- `DEBUG`: Enable/disable debug mode (default: True)
- `CACHE_TIMEOUT`: Cache expiration time in seconds (default: 3600)
- `MAX_UPLOAD_SIZE`: Maximum file size in MB (default: 10)



## Key Features

### 1. Pandas Data Type Inference and Conversion
- Advanced data type inference using rule-based methods and an AI model (spaCy)
- Smart handling of mixed data types and non-standard formats
- Support for complex data patterns through AI analysis
- Optimization for large datasets with chunk processing
- Handling of common data type issues:
  - Columns defaulting to 'object' type
  - Mixed data types in single columns
  - Non-standard date formats
  - Special values and null representations

### 2. Supported Data Types
- Text (`object`)
- Numbers (`int8`, `int16`, `int32`, `int64`, `float32`, `float64`)
- Dates (`datetime64[ns]`)
- Boolean
- Category
- Complex mixed types

### 3. Special Value Handling
- Intelligent handling of:
  - NULL values
  - Missing data
  - Currency formats
  - Percentage values
  - Mixed date formats
  - Special characters

### 4. Performance Optimization
- Memory usage optimization through data type downcasting
- Chunk processing for large files
- Caching mechanisms for processed data
- Efficient data type conversion strategies

### 5. Error Handling
- Comprehensive error detection and reporting
- Graceful fallback mechanisms
- Detailed logging system
- User-friendly error messages

## Technical Architecture

### Backend (Django REST Framework)
- RESTful API endpoints
- File processing system
- Data type inference engine
- AI model integration (spaCy)
- Statistical analysis

### Frontend (React)
- Intuitive file upload interface
- Real-time data preview
- Type modification capabilities
- Statistical visualization
- Export functionality

## Known Limitations

- Maximum file size: 10MB
- SpaCy model requires significant memory
- Some complex date formats may not be detected